...
def batch_index(length, batch_size, n_iter=100, is_shuffle=True):
    ...
    for j in list(range(n_iter)):
        if is_shuffle:
            np.random.shuffle(index)
        for i in list(range(int(length / batch_size) + (1 if length % batch_size else 0))):
...
def extract_aspect_to_id(input_file, aspect2id_file):
    dest_fp = open(aspect2id_file, 'w', encoding="utf-8")
...
def extract_word_to_id(input_file, word2id_file):
    dest_fp = open(word2id_file, 'w', encoding="utf-8")
    lines = open(input_file, encoding="utf-8").readlines()
    targets = set()
    for i in range(0, len(lines), 3):
        target = lines[i].lower().split(' ')
        for word in target:
            word = word.replace("\n","")
            if word!= '$t$' and word != '':
                targets.add(word)
    word2id = list(zip(targets, range(1, len(lines) + 1)))
    for k, v in word2id:
        dest_fp.write(k + ' ' + str(v) + '\n')
