...
tf.app.flags.DEFINE_integer('n_hidden', 600, 'number of hidden unit')
tf.app.flags.DEFINE_float('learning_rate', 0.01, 'learning rate')
tf.app.flags.DEFINE_integer('n_class', 3, 'number of distinct class')
tf.app.flags.DEFINE_integer('max_sentence_len', 120, 'max number of tokens per sentence')
...
#             cost = - tf.reduce_mean(tf.cast(self.y, tf.float32) * tf.log(prob)) + sum(reg_loss)
        ...
        with tf.Session() as sess:
            title = '-b-{}r-{}l2-{}sen-{}dim-{}h-{}c-{}'.format(
                FLAGS.batch_size,
                FLAGS.learning_rate,
                FLAGS.l2_reg,
            ...
            summary_acc = tf.summary.scalar('acc' + title, acc_)
            ...
            for i in range(self.n_iter):
                ...
                ty, py = None, None
                flag = True
                for test, num in self.get_batch_data(te_x, te_sen_len, te_x_bw, te_sen_len_bw, te_y, 3062, 1.0):
                    _loss, _acc, _summary, _step, ty, py  = sess.run([cost, accuracy, validate_summary_op, global_step,  true_y, pred_y], feed_dict=test)
